...
export class MusicStoreHeader extends LitElement {
  ...
  render() {
    return html`
      ...
      <ul>
        <li href="http://localhost:5000/home">
          <img src="http://localhost:5000/assets/ng-conf.png" width="50">
          <p class="music">Music</p><p class="store">Store</p>
        </li>
        <li>
          <a class="${window.location.pathname === '/home' ? 'is-active' : ''}" href="http://localhost:5000/home">HOME</a>
        </li>
        <li>
          <a class="${window.location.pathname === '/albums' ? 'is-active' : ''}" href="http://localhost:5000/albums">ALBUMS</a>
        </li>
        <li>
          <a class="${window.location.pathname === '/events' ? 'is-active' : ''}" href="http://localhost:5000/events">EVENTS</a>
        </li>
        <li style="${window.location.pathname === '/orders' ? 'display: block' : 'display: none'}">
          <a class="${window.location.pathname === '/orders' ? 'is-active' : ''}" href="http://localhost:5000/orders">ORDERS</a>
        </li>
      </ul>
    `;
  }
